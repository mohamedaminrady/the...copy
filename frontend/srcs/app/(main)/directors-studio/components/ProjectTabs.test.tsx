...
describe("ProjectTabs", () => {
  ...
  it("renders the characters tab when clicked", () => {
    renderWithClient(
      <ProjectTabs scenes={mockScenes} characters={mockCharacters} />
    );
    fireEvent.click(screen.getByTestId("tab-characters"));
    expect(screen.getByTestId("card-character-tracker")).toBeInTheDocument();
  });
  ...
  it("renders a message when there are no characters", () => {
    ...
    fireEvent.click(screen.getByTestId("tab-characters"));
    expect(screen.getByText(/لا توجد شخصيات بعد/)).toBeInTheDocument();
  });
});
